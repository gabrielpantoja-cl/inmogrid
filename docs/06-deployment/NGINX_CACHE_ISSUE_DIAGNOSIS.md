...
ssh gabriel@VPS_IP_REDACTED "ss -tulnp | grep -E ':(80|443|3000)'"
...
curl -k -H "Host: degux.cl" https://VPS_IP_REDACTED/ | grep 'application-name'
...
```bash
ssh gabriel@VPS_IP_REDACTED "curl -s http://localhost:3000/ | grep 'application-name'"
...
ssh gabriel@VPS_IP_REDACTED "ls -la /etc/nginx/sites-available/"
...
ssh gabriel@VPS_IP_REDACTED "ls -la /etc/nginx/sites-enabled/"
...
scp ~/Documentos/degux.cl/nginx-degux.conf gabriel@VPS_IP_REDACTED:~/
...
```bash
# Desde máquina local
curl -k -H "Host: degux.cl" https://VPS_IP_REDACTED/ | grep -E '(title|BUILD_ID|application-name)'
```

### Verificar qué está sirviendo PM2 (correcto)

```bash
ssh gabriel@VPS_IP_REDACTED "curl -s http://localhost:3000/ | grep -E '(title|application-name)'"
```

### Ver logs de Nginx en tiempo real

```bash
ssh gabriel@VPS_IP_REDACTED "sudo tail -f /var/log/nginx/access.log"
ssh gabriel@VPS_IP_REDACTED "sudo tail -f /var/log/nginx/error.log"
```

### Ver configuración activa de Nginx

```bash
ssh gabriel@VPS_IP_REDACTED "sudo nginx -T | grep -A 30 'server_name.*degux'"
```

### Verificar procesos escuchando puertos

```bash
ssh gabriel@VPS_IP_REDACTED "sudo netstat -tulnp | grep -E ':(80|443|3000)'"
```
...
```dns
;; A Records
api.degux.cl.   1  IN  A  VPS_IP_REDACTED  ; cf_tags=cf-proxied:false
degux.cl.       1  IN  A  VPS_IP_REDACTED  ; cf_tags=cf-proxied:true
www.degux.cl.   1  IN  A  VPS_IP_REDACTED  ; cf_tags=cf-proxied:true
...
5. Verificar: `dig +short degux.cl` debería mostrar `VPS_IP_REDACTED`
...
```bash
ssh gabriel@VPS_IP_REDACTED "sudo cat /etc/nginx/sites-available/default"
```
